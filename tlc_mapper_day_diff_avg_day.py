...
def create_travel_time_map():
    ...
    con = duckdb.connect("tlc_database_avg_diff.duckdb")
    ...
    if not table_exists:
        con.sql(f"""
            ...
            cell_id,
            boundary,
            MAX(CASE WHEN day_of_week = 'Monday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_monday,
            MAX(CASE WHEN day_of_week = 'Tuesday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_tuesday,
            MAX(CASE WHEN day_of_week = 'Wednesday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_wednesday,
            MAX(CASE WHEN day_of_week = 'Thursday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_thursday,
            MAX(CASE WHEN day_of_week = 'Friday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_friday,
            MAX(CASE WHEN day_of_week = 'Saturday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_saturday,
            MAX(CASE WHEN day_of_week = 'Sunday' THEN trip_count - avg_trip_count ELSE 0 END) as cnt_sunday
            FROM (
            SELECT 
                cell_id,
                boundary,
...
if __name__ == "__main__":
    ...
    map_1.save_to_html(file_name='db_big_avg_day_diff.html')